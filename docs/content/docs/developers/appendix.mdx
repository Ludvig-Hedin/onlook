...
- The design sidebar tabs can be opened from the keyboard with `Alt+1` through `Alt+5`.
- The Layers tree uses frame-aware node ids so it can render reliably when multiple frames contain overlapping DOM ids.
