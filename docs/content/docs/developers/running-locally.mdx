...
This guide will set you up with a local development environment for Weblab. This involves a Next.js app, a Supabase backend, and the ability to run dev containers with Codesandbox.

> **Note**: This guide is for development and contributing to Weblab. For deploying Weblab for production use, see [Self-Hosting](/self-hosting).

## Prerequisites

These are the prerequisites for setting up Weblab development environment.
...
### 1. Clone the repository
   ...
   ```bash
   git clone https://github.com/weblab.build.git
   cd onlook
   bun install
   ```
...
If you are only trying to get into the app quickly in development, the `DEV MODE: Sign in as demo user` button will now create the demo auth user automatically if it does not already exist, then sign you in.
...
Now that you have Weblab running locally, explore these resources to make your first contribution or learn more about the architecture. Understanding how Weblab works will help you contribute more effectively to this visual editor for React and TailwindCSS.
