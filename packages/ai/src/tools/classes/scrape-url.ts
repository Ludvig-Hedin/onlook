...
export class ScrapeUrlTool extends ClientTool {
    ...
    static readonly description =
        'Scrape a URL and extract its content in various formats (markdown, HTML, JSON, branding). Can extract clean, LLM-ready content from any website, handling dynamic content and anti-bot mechanisms. The branding format extracts brand identity information including colors, fonts, typography, spacing, and UI components. You can request multiple formats at once (e.g., both markdown and branding) to get both content and brand identity in a single call.';
    static readonly parameters = z.object({
        ...
        formats: z
            ...
            .default(['markdown'])
            .describe(
                'The formats to return the scraped content in. Defaults to markdown. Use "branding" to extract brand identity (colors, fonts, typography, etc.). You can specify multiple formats (e.g., ["markdown", "branding"]) to get both content and brand identity in a single call.',
            ),
        onlyMainContent: z
            .boolean()
            .default(true)
