...
describe('ImageContext', () => {
    const createMockImageContext = (
        overrides: Partial<ImageMessageContext> = {},
    ): ImageMessageContext => ({
    ...
    describe('getLabel', () => {
        ...
        test('should handle very long displayName', () => {
            const longName =
                'Very long image filename that might be generated by some applications ' +
                'a'.repeat(100) +
                '.png';
            const context = createMockImageContext({
    ...
    describe('toFileUIParts', () => {
        ...
        test('should handle images with empty content', () => {
            const images = [createMockImageContext({ content: '' })];
            const parts = ImageContext.toFileUIParts(images);
